        ...
        <a href="{{ route('position.index')}}" class="btn btn-primary">Back</a>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <form class="row g-3" method="POST" action="{{ route('position.update', $position->id)}}">
              @csrf
              @method('PUT')
              <div class="row">
              <div class="col-md-6">
                <label for="inputName5" class="form-label">Position Name</label>
                  <input type="text" class="form-control" name="name" placeholder="Your Name" value="{{$position->name}}">
                </div>
                <div class="col-6">
