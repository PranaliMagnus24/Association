...
<body>
 ...
 <!---Add Member--->
        ...
        <a href="{{ url('admin/cms')}}" class="btn btn-primary">Back</a>
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Edit new page</h5>
              <form class="row g-3" method="POST" action="{{ route('cms.update', $data->id)}}" enctype="multipart/form-data">
              @csrf
                <div class="col-md-12">
                <label for="inputName5" class="form-label">Page Title</label>
                  <input type="text" class="form-control" name="title" placeholder="Your Title" value="{{$data->title}}">
                           @error('title')
                ...
                <textarea class="form-control" placeholder="Small introduction" id="floatingTextarea" style="height: 100px;" name="introduction">{{$data->introduction}}</textarea>
                           @error('introduction')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                </div>
                ...
                <label for="inputName5" class="form-label">Meta Title</label>
                  <input type="text" class="form-control" name="metatitle" placeholder="Your Title" value="{{$data->metatitle}}">
                            @error('metatitle')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                </div>
                ...
                <textarea class="form-control" placeholder="Description" id="floatingTextarea" style="height: 100px;" name="metadescription">{{$data->metadescription}}</textarea>
